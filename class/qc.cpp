...
void CMDLDecompiler::QC_GenerateScript()
{
	...
	char	path[_MAX_PATH];
	char	originalName[sizeof(m_pstudiohdr->name) + 1];
	...
	size_t headerCopyLength = 0;
	bool headerLooksValid = false;
	while (headerCopyLength < sizeof(m_pstudiohdr->name) && m_pstudiohdr->name[headerCopyLength] != '\0')
	{
		++headerCopyLength;
	}
	if (headerCopyLength == 0 && ModelPath[0] != '\0')
	{
		strncpy(originalName, ModelPath, sizeof(originalName) - 1);
		originalName[sizeof(originalName) - 1] = '\0';
	}
	else
	{
		if (headerCopyLength == sizeof(m_pstudiohdr->name))
		{
			headerCopyLength = sizeof(m_pstudiohdr->name);
		}
		for (size_t idx = 0; idx < headerCopyLength; ++idx)
		{
			unsigned char raw = static_cast<unsigned char>(m_pstudiohdr->name[idx]);
			if (raw == '.' && idx + 3 < sizeof(m_pstudiohdr->name))
			{
				unsigned char m = static_cast<unsigned char>(m_pstudiohdr->name[idx + 1]);
				unsigned char d = static_cast<unsigned char>(m_pstudiohdr->name[idx + 2]);
				unsigned char l = static_cast<unsigned char>(m_pstudiohdr->name[idx + 3]);
				if (tolower(m) == 'm' && tolower(d) == 'd' && tolower(l) == 'l')
				{
					headerLooksValid = true;
				}
			}
			if (raw < 32 || raw == 127 || raw > 126)
			{
				originalName[idx] = '?';
			}
			else
			{
				originalName[idx] = static_cast<char>(raw);
			}
		}
		originalName[headerCopyLength] = '\0';
	}

	const char *reportedName = headerLooksValid ? originalName : (ModelPath[0] ? ModelPath : ModelName);
	fprintf(qcfile,"Original model name: %s\n\n",reportedName);
	fprintf(qcfile,"==============================================================================\n");
	fprintf(qcfile,"*/\n\n");

	//main
	//fprintf(qcfile,"$modelname \"%s%c%s.mdl\"\n",path,PATH_SEPARATOR,ModelName);
	fprintf(qcfile,"$modelname \"%s.mdl\"\n",ModelName);
	fprintf(qcfile,"$cd \"%s%c\"\n",path,PATH_SEPARATOR);
	fprintf(qcfile,"$cdtexture \"%s%c\"\n",path,PATH_SEPARATOR);
